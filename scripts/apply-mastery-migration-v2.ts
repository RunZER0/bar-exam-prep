/**
 * Apply mastery engine migration using raw SQL execution
 * Run with: npx tsx scripts/apply-mastery-migration-v2.ts
 */
...
async function applyMigration() {
  ...
  try {
    ...
    const statements = [
      // ENUMS
      `DO $$ BEGIN
        CREATE TYPE format_tag AS ENUM ('mcq', 'written', 'oral', 'drafting', 'flashcard');
      EXCEPTION WHEN duplicate_object THEN null; END $$`,
      
      `DO $$ BEGIN
        CREATE TYPE item_type AS ENUM ('mcq', 'issue_spot', 'essay', 'oral_prompt', 'drafting_task', 'flashcard');
      EXCEPTION WHEN duplicate_object THEN null; END $$`,
      
      `DO $$ BEGIN
        CREATE TYPE attempt_mode AS ENUM ('practice', 'timed', 'exam_sim');
      EXCEPTION WHEN duplicate_object THEN null; END $$`,
      
      `DO $$ BEGIN
        CREATE TYPE exam_phase AS ENUM ('distant', 'approaching', 'critical', 'exam_week', 'post_exam');
      EXCEPTION WHEN duplicate_object THEN null; END $$`,
      
      `DO $$ BEGIN
        CREATE TYPE difficulty_level AS ENUM ('foundation', 'core', 'advanced');
      EXCEPTION WHEN duplicate_object THEN null; END $$`,
      ...
      `DO $$ BEGIN
        CREATE TYPE mapping_strength AS ENUM ('primary', 'secondary');
      EXCEPTION WHEN duplicate_object THEN null; END $$`,
