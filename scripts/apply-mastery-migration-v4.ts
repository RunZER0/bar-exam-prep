 ...
 * Run with: npx tsx scripts/apply-mastery-migration-v4.ts
 */
...
async function applyMigration() {
  ...
  try {
    ...
    const existingEnums = await sql`
      ...
      GROUP BY typname
    `;
    console.log('Existing enums:', existingEnums.map(e => `${e.typname}(${e.labels})`).join(', '));

    // ENUMS - conditionally create only missing ones
    console.log('\nCreating ENUMs (if not exist)...');
    
    const enumDefs = [
      { name: 'format_tag', values: ['mcq', 'written', 'oral', 'drafting', 'flashcard'] },
      { name: 'item_type', values: ['mcq', 'issue_spot', 'essay', 'oral_prompt', 'drafting_task', 'flashcard'] },
      { name: 'attempt_mode', values: ['practice', 'timed', 'exam_sim'] },
      { name: 'exam_phase', values: ['distant', 'approaching', 'critical', 'exam_week', 'post_exam'] },
      { name: 'mastery_difficulty', values: ['foundation', 'core', 'advanced'] }, // Renamed to avoid conflict
      { name: 'error_severity', values: ['minor', 'moderate', 'major'] },
      { name: 'mapping_strength', values: ['primary', 'secondary'] },
    ];

    for (const e of enumDefs) {
      const exists = existingEnums.some(ex => ex.typname === e.name);
      if (!exists) {
        const valuesStr = e.values.map(v => `'${v}'`).join(', ');
        await sql.unsafe(`CREATE TYPE ${e.name} AS ENUM (${valuesStr})`);
    ...
    // For micro_skills, use TEXT for difficulty to avoid enum issues
    await sql.unsafe(`
